 ...
 */
//해결책 중 하나는 2개의 참조를 사용해, Node를 순회하는 것이다.
//하나의 참조는 다른 것 보다 2배 빠르다. 빠른 참조가 끝에 도달하면, 느린 참조가 중간에 있게 된다.
...
func getMiddle<T>(_ list: LinkedList<T>) -> Node<T>? {
    ...
    while let nextFast = fast?.next {
        fast = nextFast.next
        //다음 node가 있는 경우, nextFast가 되어 slow보다 2배 빠르게 순회한다.
        //fast는 두 번 업데이트 된다.
...
//이 알고리즘의 시간 복잡도는 끝까지 순회를 한 번 하기 때문에 O(n)이다.
