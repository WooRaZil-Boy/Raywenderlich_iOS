...
//그 중 하나는 연속적이고 순서가 지정된 요소를 list로 저장하는 자료구조인 Array이다. p.95
...
//하지만, 이 작업은 자주 발생하지 않기 때문에, 평균적인 비용은 O(1)이 된다. p.97
...
//또한 새 요소를 만들 때마다 상대적으로 큰 비용의 동적 할당이 필요하다.
...
//Strengths and weaknesses
//QueueRingBuffer 기반 Queue를 요약하면 다음과 같다. p.107
// • enqueue : 평균 - O(1), 최악 - O(1)
// • dequeue : 평균 - O(1), 최악 - O(1)
// • space complexity : 평균 - O(n), 최악 - O(n)
//QueueRingBuffer는 enqueue, dequeue에서 QueueLinkedList와 동일한 시간 복잡도를 가진다.
//유일한 차이점은 공간 복잡도이다. RingBuffer의 크기는 고정되어 있어, 늘릴 수 없다.
...
//Swift는 저장 공간이 부족할 때마다 매번 2배로 늘리기 때문에 이러한 경우는 자주 발생하지는 않는다.
