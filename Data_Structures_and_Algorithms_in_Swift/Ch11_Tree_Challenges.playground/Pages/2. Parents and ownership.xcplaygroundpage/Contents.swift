...
//Root에는 parent가 없으므로 optional로 선언해야 한다. reference cycle을 피하기 위해 weak으로 선언한다.
//일반적으로 node는 하위 항목들과는 강력한 소유 관계(strong ownership relationship)를 가지지만, 상위 항목과는 관계가 약하다(weak).
//이렇게 parent를 포함하는 node는 이중 연결 리스트(doubly linked list)와 유사하다.
