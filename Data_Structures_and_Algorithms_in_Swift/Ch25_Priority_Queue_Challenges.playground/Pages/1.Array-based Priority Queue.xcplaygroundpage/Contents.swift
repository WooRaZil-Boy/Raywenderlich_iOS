...
//배열로 구현한 우선 순위 큐
//우선 순위 큐는 우선 순위에 따라 요소를 Queue에서 dequeue한다. min 혹은 max priority queue 가 될 수 있다.
...
//추가하는 새 요소에 대해 우선 순위를 확인하기 위해 모든 요소를 거쳐야 하므로 O(n)의 시간 복잡도이다.
//또한 Array의 요소 사이에 insert 하는 경우, 요소를 오른쪽으로 하나씩 이동해야 한다.
//Heap처럼 트리 구조를 생각하지 않고 단순히 Array로 생각해야 한다.
//dequeue를 구현한다.
extension PriorityQueueArray {
...
//Array의 요소를 제거하기 전에 Queue가 비어있는 지 확인해야 한다. dequeue 이후, 기존 요소를 왼쪼긍로 하나씩 이동해야 하므로 O(n) 연산이다.
//마지막으로 우선 순위 큐의 출력을 위해, CustomStringConvertible를 구현한다.
