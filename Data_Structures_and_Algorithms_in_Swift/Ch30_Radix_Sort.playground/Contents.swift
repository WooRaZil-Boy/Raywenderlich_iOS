...
//다음으로 다음 자리수(10의 자리)를 기준으로 위의 절차를 반복한다. (1 : 410 | 2 : 20 | 7 : 1772 | 8 : 88)
...
//ex. 100자리수에서 0 bucket의 경우, 20이 88보다 먼저 온다. 이전 단계에서 20을 80보다 낮은 bucket에 넣었으므로, 20이 88보다 먼저 오게 된다.
...
//Key points
// • 이전에 구현한 다른 검색 알고리즘과 달릭, 기수 정렬(radix sort)은 두 값을 비교하는데 의존하지 않는 non-comparative 정렬 방법이다.
//  기수 정렬(radix sort)은 값을 걸러내기 위해 필터와 같은 bucker을 사용한다. 자판기에서 동전을 크기에 따라 구별하는 것과 비슷하다.
// • 기수 정렬(radix sort)은 위치 표기법(positional notation)으로 값을 정렬하기 위한 가장 빠른 정렬 알고리즘(sorting algorithm) 중 하나이다.
// • 여기서는 최소 자리수(least significant digit, LSD)로 구현했지만, 최대 자리수(most significant digit, MSD)로 구현할 수도 있다.
