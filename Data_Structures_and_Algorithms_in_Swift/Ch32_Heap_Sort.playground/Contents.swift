...
//힙 정렬은 Heap을 사용하여 오름차순으로 Array를 정렬하는 또다른 비교 기반(comparison-based) 알고리즘이다.
...
//Getting started
//Heap을 확장하여 정렬에 사용할 수 있다.




//Example
//정렬되지 않은 Array의 경우, 오름차순으로 정렬하려면 먼저 이 Array를 최대 힙(max heap)으로 변환하애 한다. //p.305
//모든 상위 node가 올바른 위치에 오도록 이동해서 최대 힙을 만든다.
//sift-down의 시간 복잡도는 O(log n)이므로, heap을 구축하는 총 시간 복잡도는 O(n log n)이다.
//이제 만들어진 Array를 오름차순으로 변경한다.
//max heap에서 가장 큰 요소(최대값)은 항상 root에 있기 때문에 index 0의 첫 요소를 index n-1의 마지막 요소와 교환하는 것으로 시작한다.
//swap의 결과로 Array의 마지막 요소는 올바른 곳에 위치하지만, heap이 성립하지 않는다. 따라서, 새로운 root가 올바른 위치에 놓일 때까지 sift down한다.
//heap이 아닌 정렬된 Array의 일부로 간주한다. 따라서 heap의 마지막 요소는 제외한다. sift down 결과 두 번째로 큰 요소가 새 root가 된다.
//이전 단계를 반복해서 정렬을 맞춰간다. 패턴은 매우 간단하다. 첫 요소와 마지막 요소를 swap하면, 더 큰 요소가 올바른 순서로 Array의 뒤로 이동한다.
//heap의 크기가 1이 될 때까지, swap과 shift를 반복하면 된다. 이 과정의 선택 정렬(selection sort)와 비슷하다.
...
//ex. 카드의 덱을 순위별로 heap sort 하는 경우, 원래 덱과 관련하여 순서가 변경될 수 있다.
