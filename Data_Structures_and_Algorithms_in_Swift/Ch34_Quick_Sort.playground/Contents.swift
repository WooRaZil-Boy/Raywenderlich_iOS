...
//Merge Sort(합병 정렬), Heap Sort(힙 정렬)는 기교 기반 정렬 알고리즘(comparison-based sorting algorithm)이다.
//Quick Sort(퀵 정렬) 또한 비교 기반 정렬 알고리즘으로, 합병 정렬과 마찬가지로 분할 정복 전략(divide and conquer)을 사용한다.
//퀵 정렬의 중요한 기능 중 하나는 Pivot(피벗)을 선택하는 것이다. 피벗은 Collection을 세 개의 부분을 나눈다. [ elements < pivot | pivot | elements > pivot ]
...
public func quicksortNaive<T: Comparable>(_ a: [T]) -> [T] {
    guard a.count > 1 else { //하나 이사으이 요소가 있어야 한다.
        return a
    }
...
//중간 요소를 피벗으로 선택하므로, 8이 된다. 이를 사용해 3개의 부분으로 분할하면 다음과 같다.
// less: [0, 3, 2, 1, 5, -1]
// equal: [8, 8]
// greater: [12, 9, 18, 27, 21]
//분할 된 3개의 각 부분이 정렬되지 않았다. 퀵 정렬은, 모든 부분에 0 개 또는 1 개의 요소만 남을 때까지 더 작은 부분으로 재귀적으로 나눈다. //p.317
//재귀가 끝나면, 나눠진 최소 부분들을 다시 결합하여 정렬된 배열이 된다.
// [-1, 1, 2, 3, 5, 8, 8, 9, 12, 18, 21, 27]
//이 Naive 구현은 이해하기 쉽지만, 몇 가지 문제가 있다.
// • 동일한 Array에서 filter를 세 번 호출하는 것은 비효율적이다.
// • 모든 부분에 대해 새 Array를 생성하는 것은 공간효율적이지 않다. 새 Array를 사용하지 않고, 제자리에서 정렬할 수 있으면 공간이 낭비되지 않는다.
...
//위의 Naive 구현을 좀 더 효율적으로 변경할 수 있다. Lomuto’s algorithm을 사용해, 분할을 효율적으로 구현할 수 있다.
...
//   i
//   j
//그리고, 첫 요소인 12를 pivot과 비교한다. pivot(8)보다 크므로 다음 요소로 넘어간다.
// [12, 0, 3, 9, 2, 21, 18, 27, 1, 5, 8, -1, | 8 ]
//  low                                       high
//   i
//      j
//다음 요소인 0은 pivot보다 작으므로, index i에 있던 12와 교체되고, i는 1 증가한다.
// [0, 12, 3, 9, 2, 21, 18, 27, 1, 5, 8, -1, | 8 ]
// low                                        high
//     i
//         j
//그 다음 요소인 3 또한, pivot보다 작으므로, swap을 한다.
// [0, 3, 12, 9, 2, 21, 18, 27, 1, 5, 8, -1, | 8 ]
// low                                        high
//         i
//            j
//이 작업을 pivot을 제외한 모든 요소에 반복한다. loop 종료 이후, 결과는 다음과 같다.
...
//아래와 같이 정렬되지 않은 Array를 예시로 들면 다음과 같다.
// [12, 0, 3, 9, 2, 21, 18, 27, 1, 5, 8, -1, 8]
//먼저 12가 pivot으로 설정된다. i(앞에서부터), j(뒤에서부터)는 array에서 각각 pivot보다 작지 않은 요소(i), pivot보다 크지않은 요소(j)를 찾는다.
...
//                        i      j
//이후에, i와 j를 이동하면 서로 겹치게 된다.
// [8, 0, 3, 9, 2, -1, 8, 5, 1, 27, 18, 21, 12]
//                           j,  i
//Hoare’s algorithm은 이 지점에서 종료된다. 두 영역 사이를 구분하는 index로 j가 반환된다. Lomuto's algorithm에 비해 swap의 수가 훨씬 적다.
...
//여기서는 중앙값의 index가 center가 되고 이를 반환한다.
...
//Lomuto’s algorithm을 사용하면, 중복값이 구분보다 작게 끝이나 그룹화되지 않는다.
//Hoare’s algorithm의 경우에는, 모든 곳에서 중복값이 존재할 수 있기 때문에 더 문제가 심각하다.
//중복 요소를 정리하는 해결책은 Dutch national flag partitioning을 사용하는 것이다. 이 방법은 빨강, 흰색, 파랑의 세 가지 색으로 나눠진 네덜란드 국기에서 이름을 가져왔다.
//이는 세 개의 구분을 만드는 것과 유사하며, Dutch national flag partitioning는 중복된 요소가 많을 때 사용하기 좋은 기술이다.
...
//     e
//                                       l
//0은 pivot보다 작으므로 swap하고, samller와 equal을 증가시킨다. //위의 if에서 if 구문
// [0, 8, 3, 9, 2, 21, 18, 27, 1, 5, 8, -1, 12]
//     s
//        e
//                                       l
//진행 중 요소들의 상태는 다음과 같다.
...
//                        e
//                            l
//여기서 27을 비교하고 있다. pivot보다 크므로 large(1)과 swap되고, lager의 index는 감소한다.
// [0, 3, -1, 2, 5, 8, 8, 1, 27, 18, 21, 9, 12]
//                  s
//                        e
//                        l
//equal과 large가 같아졌지만, loop 조건은 equal <= larger 이므로 계속 진행된다.
//equal의 요소는 아직 비교되지 않았다. 여기서는 1으로 pivot보다 작으므로 8(smaller)와 swap되고, smaller의 index가 증가한다.
// [0, 3, -1, 2, 5, 1, 8, 8, 27, 18, 21, 9, 12]
//                     s
//                           e
//                        l
//smaller와 larger가 중간 부분의 첫 요소와 마지막 요소를 가리키게 된다. 이를 반환해서 세 부분의 경계를 명확하게 할 수 있다.
