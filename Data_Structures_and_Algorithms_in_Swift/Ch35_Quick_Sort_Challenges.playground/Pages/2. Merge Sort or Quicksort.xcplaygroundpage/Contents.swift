 ...
 */
//퀵 정렬(quick sort)보다, 합병 정렬(merge sort)을 사용해야 하는 때와 그 이유를 설명한다.
...
//      Stable Sort (안정 정렬) : 동일한 값에 대해 기존의 순서가 유지되는 정렬 방식 (버블 정렬, 삽입 정렬)
//      Unstable Sort (불안정 정렬) : 불안정 정렬 - 동일한 값에 대해 기존의 순서가 유지되지 않는 정렬 방식 (선택 정렬)
