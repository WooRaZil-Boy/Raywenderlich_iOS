...
//이전 장에서 다음 level로 넘어 가기 전에 해당 정점의 모든 인접 정점을 탐색해야 하는 너비 우선 탐색(Breadth-First Search, BFS)을 살펴 보았다.
...
//DFS를 수행하려면, 주어진 source 정점에서 시작하여 끝까지 도달할 때까지 가능한 하나의 분기를 탐색해야 한다.
//원하는 정점을 찾거나, 모든 정점을 방문할 때까지 역추적(backtrack, 한 단계 뒤로 이동한다)한 후, 가능한 다음 분기를 탐색한다.




//Example
//이전 장과 같은 Graph를 예시로 사용한다. 이를 통해 BFS 와 DFS의 차이점을 알 수 있다. //p.378
//: ![sampleGraph](sampleGraph.png)
//DFS에서는 Stack을 사용해, 이동하는 level을 추적한다. Stack의 후입선출(last-in-first-out) 특성은 backtracking에 도움이 된다.
//Stack에 push할 때마다, 한 level 더 깊이 이동한다는 것을 의미한다. 막 다른 경로에 도달하면, pop 해서 이전 level로 돌아갈 수 있다.
// 1. 이전 BFS 구현과 같이, 시작 정점으로 A를 선택한다. A를 stack에 추가한다.
// 2. Stack이 비어 있지 않다면, Stack의 가장 위 정점을 방문하고, 해당 정점에서 아직 방문하지 않은 첫 번째 인접 정점을 push한다.
//  이 경우에는 A를 방문하고, B를 push한다.
//  이제, Stack은 [A, B]가 된다.
//  간선을 추가한 순서가 검색 결과에 영향을 미친다. 이 경우에는 A에서 추가된 첫 번째 간선이 B를 연결하는 간선이므로 B가 먼저 push 된다.
// 3. A는 이미 방문했으므로, B를 방문하고 E를 Stack에 push한다.
//  이제, Stack은 [A, B, E]가 된다.
// 4. E를 방문하고, F를 Stack에 push한다.
//  이제, Stack은 [A, B, E, F]가 된다.
//  Stack에 push할 때마다, 해당 분기를 더 멀리까지 탐색하게 된다.
...
//정점을 탐색할 때 방문한 경로를 보여주는 tree를 구성할 수 있다. BFS와 비교하여 DFS가 얼마나 깊이 탐색하는 지 알 수 있다. //p.381
...
//DFS로 그래프를 탐색할 때, 인접 정점을 모두 확인해야 방문할 수 있는 정점을 찾을 수 있다.
//최악의 경우 그래프의 모든 단일 간선을 방문해야 하기 때문에 이 작업의 시간 복잡도는 O(E)가 된다.
//따라서 전체적인 깊이 우선 검색(Depth-First Search, DFS)의 시간 복잡도는 O(V + E)이다.
//DFS의 공간 복잡도는 정점을 stack, pushed, visited의 세 가지 자료 구조로 저장해야 하기 때문에 O(V)이다.
...
// • DFS는 분기의 끝에 도달할 때까지 가능한 한 끝까지 한 경로를 탐색한다.
