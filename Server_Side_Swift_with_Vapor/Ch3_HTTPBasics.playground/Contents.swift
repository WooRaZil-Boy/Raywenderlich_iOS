...
//HTTP responses
//서버는 요청을 처리할 때 HTTP response로 return한다.
...
//많은 작업을 해야 하지만, 유연하게 필요한 것을 정확히 개발할 수 있다.
...
//여러 스레드를 생성하고 이들 사이를 전환하면 추가적인 오버헤드가 발생한다.
//fully non-blocking 아키텍처에서는 request가 전달되고 다른 request는 원래 request의 결과가 준비될 때까지 처리된다.
//결과적으로 성능이 우수하다.
