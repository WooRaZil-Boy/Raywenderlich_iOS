...
//컴퓨터는 문자열을 개별 문자의 모음(character set)으로 생각한다. 프로그래밍 언어는 모든 코드를 원시 숫자로 변환할 수 있다.

//Unicode
//공통 문자 집합의 표준. p.56. emoji도 유니코드에 있다("emoji"는 일본어로, "e"는 그림을 의미하고 "moji"는 문자를 의미한다).
...
//Swift에서는 단일 문자를 저장하더라도, Type inference에서는 String을 할당한다. Character로 사용하려면 명시적으로 지정해 줘야 한다.
...
//Tuples
//좌표와 같이 쌍으로 된 데이터를 나타낼 때, 튜플을 사용해 간단히 표현할 수 있다. 튜플은 여러 유형의 값으로 구성된 데이터를 나타내는 유형이다.
...
//코드가 다른 소프트웨어와 상호작용해야 할 때나, 메모리를 최적화해야 되는 특수한 경우를 제외한 대부분의 경우에는 Int를 사용하는 것을 권장한다. 각 유형과 그 크기에 대한 정보는 p.64
//Double보다 작은 범위와 정밀도를 가진 Float 형이 있지만, 최신 하드웨어는 Double에 최적화되어 있기 때문에 Double을 사용하는 것을 권장한다. p.64
...
//Swift의 훌륭한 특징 중 하나는 Protocol을 사용하여 Type의 공통성을 유지한다는 것이다. p.65
//Swift는 최초의 Protocol-based language 이다. Protocol을 이해하면, 다른 언어에서 가능하지 않은 방식으로 시스템을 활용할 수 있다.
