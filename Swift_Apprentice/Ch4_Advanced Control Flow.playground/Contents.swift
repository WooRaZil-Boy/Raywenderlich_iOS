...
//A Random Interlude
//arc4random() 대신 Swift 고유의 난수 생성 메서드가 Swift 4.2 이후 부터 추가됐다.
...
for _ in 0..<count { //일반적으로 loop에서 halfOpenRange를 사용하는 경우가 많다.
    //half-open range (..<)를 사용하면, 반복회수는 count-1 이 된다.
    let temp = sum
    sum = sum + lastSum
    lastSum = temp
}
...
//where절로 Bool 조건을 추가한다. true일 때만 코드 블록을 실행한다.

//Continue and labeled statements
//loop를 완전히 벗어나지 않고 특정 조건에만 루프를 건너 뛰고 싶을 경우 continue를 사용할 수 있다. 현재 loop를 즉시 종료하고 다음 loop를 시작한다.
...
//루프에 레이블을 지정해 줄 수 있다(labeled statement). 이 레이블을 continue와 결합해 사용할 수 있다.
...
//정수 이외의 type으로도 Switch 문을 만들 수 있다. 여기서는 String을 사용한다.
...
//break를 써주지 않으면 해당 조건에 일치하는 모든 case를 실행하는 언어들도 있지만,
//Swift는 여러 case의 조건에 만족하는 경우, 일치하는 첫 case에서만 실행된다.
switch number {
...
//let where를 사용해 bool 조건을 pattern matching 해 줄 수 있다. true인 경우만 바인딩한다.
switch number {
case _ where number % 2 == 0: //해당 값을 사용하지 않는 다면 underscore를 사용해 줄 수 있다.
    print("Even")
default:
    print("Odd")
}
//pattern matching한 결과값을 활용하지 않는 경우 생략해 줄 수 있다.
//Swift 예전 버전에서는 let _ 로 사용하지 않아도 선언을 해 줘야 했지만, Swift 버전이 업데이트 되면서 underscore만 사용하면 되도록 변경되었다.
...
case let (x, y, z): //default 역할을 한다. //(let x, let y, let z)와 같다.
