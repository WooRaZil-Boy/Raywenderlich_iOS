...
//Big-O notation은 실행 시간 또는 작업이 완료되는 데 걸리는 시간을 설명하는 방법이다. 정확한 시간을 구하는 것이 아니라 상대적인 값을 구한다.
//데이터 양에 상관없이 항상 일정한 상수의 시간이 소요된다면 O(1)이 된다. 데이터 양이 늘어날 때 선형적으로 소요 시간이 증가한다면 O(N)이다.
...
//mutable, immutable의 개념을 먼저 생각해 봐야 한다. 다른 Type을 생성 했을 때와 마찬가지로 Collection을 생성할 때 상수 또는 변수로 선언해야 한다.
...
//What is an array?
//배열은 같은 Type의 정렬된 값들의 모음이다. zero-index부터 시작하므로, 마지막 index는 값의 개수 -1이 된다. p.127
//동일한 값이 여러 번 포함될 수 있다.

//When are arrays useful?
//배열은 항목을 특정 순서로 저장하는 경우 유용하다. 전체 배열을 반복하지 않고 index로 요소를 가져 올 수 있다.
...
var subscribers: [String] = [] //빈 배열 선언 //빈 배열의 경우에는 Type inference를 적용할 수 없으므로 Type을 명시적으로 지정해 줘야 한다.
let allZeros = Array(repeating: 0, count: 5) // [0, 0, 0, 0, 0] //default value를 사용해 같은 값을 반복해서 가지는 배열을 생성할 수 있다.
let vowels = ["A", "E", "I", "O", "U"] //상수로 선언되면 변경할 수 없다.
...
currentPlayer = players.min() //function //사전 순에서 먼저 나올 수록 값이 낮아 min이 된다. //index가 아닌 value를 기준으로 한다. //반대 작업을 하는 max()도 있다.
...
//subscript는 non-optional type을 반환한다. 존재하지 않는다면 nil을 반환하는 것이 아니라 런타임 오류가 발생한다.

//Using countable ranges to make an ArraySlice
//countable range를 사용한 subscript syntax를 사용하여 배열에서 하나 이상의 값을 가져올 수 있다.

let upcomingPlayersSlice = players[1...2] //index가 1...2 인 요소를 가져온다. //upcomingPlayersSlice의 Type은 Array가 아니라 원래 배열의 ArraySlice이다.
//시작 값이 최종 값보다 작거나 같고, 배열 범위 내에 있는 index인 경우 사용할 수 있다. //값을 복사해 저장한다. 따라서 이후 배열의 값이 변화해도 영향 받지 않는다.
print(upcomingPlayersSlice[1], upcomingPlayersSlice[2])
// > "Bob Cindy\n"

let upcomingPlayersArray = Array(players[1...2]) //Array로 변환한다.
...
func isEliminated(player: String) -> Bool {
    return !players.contains(player) //contains(_:)는 해당 배열에 특정 요소가 하나 이상 존재하는 지 여부를 반환한다.
}
...
//Modifying arrays
//Appending elements
players.append("Eli") //append(_:)로 요소를 배열 뒤에 추가해 줄 수 있다(mutable에서만 작동). //배열은 같은 유형만 저장 가능하므로 Type을 맞춰줘야 한다.
...
print(players.index(of: "Dan") as Any) //해당 요소의 index 반환. 요소가 중복되어 있으면, 첫 번째 index를 반환하고, 요소를 찾지 못하면 nil을 반환한다.

//Updating elements
print(players)
// > ["Alice", "Bob", "Dan", "Eli", "Frank"]
players[4] = "Franklin" //update 하려는 요소를 제거하고 다시 추가할 수도 있지만, subscript syntax로 쉽게 구현할 수 있다. //배열의 범위를 벗어나는 index를 사용하면 런타임 오류가 난다.
print(players)
// > ["Alice", "Bob", "Dan", "Eli", "Franklin"]

players[0...1] = ["Donna", "Craig", "Brian", "Anna"] //countable range를 subscript syntax로 사용해 여러 값을 동시에 update할 수도 있다.
...
//배열은 연속적인 block으로 메모리에 저장된다(ex. 10 개의 요소가 있으면, 10 개의 값이 모두 다른 요소로 나란히 저장된다. 따라서 배열의 성능(performance cost)은 다음과 같다.
//Accessing elements : 요소에 접근하는 비용은 크지 않다. 배열의 크기에 상관없이 subscript로 접근하므로 fetching에는 O(1)의 비용이 든다.
//Inserting elements : 요소를 삽입하는 비용은 요소를 삽입하려는 위치에 따라 다르다.
//  • 배열의 앞 부분에 삽입하는 경우 : 삽입하려는 요소를 위한 공간을 생성하고, 모든 요소를 하나씩 뒤로 이동해야 되기 때문에 배열의 크기에 비례한 시간이 필요하다.
//  이를 선형 시간이라 하며 O(n)의 비용이 든다.
...
//  하지만 배열에 남은 공간이 없다면, 더 큰 크기의 배열을 만들고 이전 배열을 복사해야 되기 때문에 O(n)이 된다.
...
//  평균적으로 n /2 가 되고 이는 선형적인 시간이므로 O(n)이 된다.




//Dictionaries
//dictionary는 정렬되지 않은 key와 value의 쌍으로 구성된다. p.138 동일한 key는 두 번 나타날 수 없지만 다른 key가 동일한 value를 가리킬 수 있다.
//모든 key는 같은 Type이어야 하며, 모든 value도 같은 Type이어야 한다.
//dictionary는 identifier를 사용해 value를 조회하는 경우에 유용하다(ex. 책의 목차를 생각하면 된다).
//배열은 index로 값을 가져온다. index는 정수여야 하며, 모든 index는 순차적이어야 한다. dictionary에서 key는 어떤 Type이든 특별한 순서가 없다.




//Creating dictionaries
//dictionary를 만드는 가장 쉬운 방법은 literal을 사용하는 것이다. 이것은 key-value 쌍의 목록이다. //쉼표로 구분되고, 대괄호로 묶인다.
var namesAndScores = ["Anna": 2, "Brian": 2, "Craig": 8, "Donna": 6] //dictionary의 type은 [String: Int]가 된다. //Type inference가 적용된다.
print(namesAndScores) //Dictionary는 Array와 달리 순서가 없기 때문에 출력할 때마다 결과가 다를 수 있다.
// > ["Craig": 8, "Anna": 2, "Donna": 6, "Brian": 2]

namesAndScores = [:] //빈 dictionary
var pairs: [String: Int] = [:] //선언과 동시에 빈 dictionary 할당 //빈 dictionary의 경우에는 Type inference를 적용할 수 없으므로 Type을 명시적으로 지정해 줘야 한다.

pairs.reserveCapacity(20) //reserveCapacity(_:)로 key-value를 저장할 공간(용량)을 설정한다. 성능이 최적화 되지만 자동으로 효율적인 할당을 하기에 크게 신경 쓸 필요 없다.
...
//Using properties and methods
//dictionary는 배열과 마찬가지로 Collection 프로토콜을 구현한다. 따라서 많은 속성과 메서드를 공유한다.
...
bobData["city"] = "San Francisco" //subscript 를 사용할 수도 있다.
...
//해쉬값을 가진 타입이 딕셔너리의 키가 될 수 있다. 프로그램 실행 시 마다 Hash 값은 달라질 수 있다.
...
//하지만 이 해시값을 저장해서 id등으로 사용해서는 안된다. 프로그램을 실행할 때마다 항상 동일하게 유지되지 않기 때문이다.
//Dictionary의 성능은 해시에 따라 달라진다. 좋지 않은 해싱 함수를 사용하면 모든 연산이 O(n)으로 성능이 저하된다. 하지만 기본 유형에 내장된 Hashable 구현은 최적화 되어 있다.
...
//Sets
//집합은 동일한 Type의 고유 한 값(중복을 허용하지 않는다)의 정렬되지 않은 Collection이다. 따라서 중복되는 값이 없어야 하고 순서가 중요하지 않을 때 매우 유용하다.

//Creating sets
let setOne: Set<Int> = [1] //꺽쇠 괄호 안에 Type을 써서 명시적으로 Set을 선언할 수 있다.

//Set literals
let someArray = [1, 2, 3, 1] //Set은 고유한 leteral이 없기 때문에 이와 같이 선언해서는 안 된다(Array로 선언 된다). //Type을 따로 지정해 주지 않으면 배열이 된다.
var explicitSet: Set<Int> = [1, 2, 3, 1] //Set을 선언하려면 명시적으로 Type을 선언해 줘야 한다.
var someSet = Set([1, 2, 3, 1]) //이렇게 Type Inference를 하도록 해 줄 수도 있다.
...
//first와 last 속성을 사용할 수도 있다. 집합의 한 요소가 반환되는데, 집합은 순서가 없기 때문에 어떤 아이템이 반환될지 알 수 없다.

//Adding and removing elements
someSet.insert(5) //insert(_:)를 사용해 집합에 요소를 추가할 수 있다. 해당 요소가 이미 있으면 메서드는 아무것도 수행하지 않는다.

let removedElement = someSet.remove(1) //remove(_:) 로 해당 요소를 집합에서 삭제해 줄 수 있다. //삭제된 값을 반환한다. 없다면 nil을 반환한다.
