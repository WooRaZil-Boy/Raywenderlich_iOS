...
//비교를 수행해서 true, false를 나타내는 Boolean 연산자가 있다.
let yes: Bool = true
let no: Bool = false
...
//Boolean logic
//하나의 조건이 아닌 다중 조건도 AND와 OR을 결합해 검사할 수 있다.
let and = true && true //AND //모두 true인 경우에만 true
let or = true || false //OR //모두 false인 경우에만 false
...
//toggle() 함수를 사용하면, Bool값이 반대로 변경된다. 반면, !switchState은 따로 값을 할당하지 않으면 값이 변경되지 않는다.
...
if 2 > 1 { //특정 조건이 true인 경우에만 코드를 실행한다.
    print("Yes, 2 is greater than 1.")
}
//else 절을 추가해 if 문을 확장해 줄 수 있다. false인 경우에 실행된다.
let animal = "Fox"
...
print(timeOfDay)
//여기서 else 절은 생략할 수 있으며, 조건이 처음 true가 되는 첫 번째 블록만 실행한다. 따라서 else if 문의 순서가 중요하다.
...
//해당 코드 블럭을 벗어나면 그 범위 안에서 선언되었던 변수, 상수를 사용할 수 없다. 하지만, 상위 코드블럭의 변수와 상수는 사용할 수 있다.
...
let min1 = a < b ? a : b
let max1 = a > b ? a : b
//min, max function은 Swift Standard Library에 있다.
...
//위 코드는 sum이 1000보다 커지기 전까지 반복된다 (9번째 loop, sum == 1023으로 종료).
...
//하지만 repeat while을 쓴 이 코드의 경우, 조건 검사가 loop의 끝에서 이루어 지므로 한번은 실행되어 값이 달라진다(sum == 3).
...
while true {
    sum = sum + (sum + 1)
    if sum >= 1000 { //sum 이 1000보다 크거나 같으면
        break //loop가 종료된다.
    }
}
//동일한 결과를 얻는 여러가지 방법이 있다. 가장 읽기 쉽고, 효율적인 코드를 선택해야 한다.
