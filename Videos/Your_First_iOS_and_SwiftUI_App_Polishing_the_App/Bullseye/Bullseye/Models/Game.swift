...
struct Game {
  ...
  func points(sliderValue: Int) -> Int {
    ...
    let bonus: Int
    if difference == 0 { //정확하게 맞춘 경우 100점 보너스
      bonus = 100
    } else if difference <= 2 { //2점 내외로 근접하게 맞췄을 경우 50점 보너스
      bonus = 50
    } else { //일반적인 경우
      bonus = 0
    }
  ...
  mutating func startNewRound(points: Int) { //struct에서 내부 멤버 값을 변경해야 하는 경우 mutating 키워드가 필요하다.
    score += points
    round += 1
