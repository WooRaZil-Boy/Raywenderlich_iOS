...
//이를 Red-Green-Refactor Cycle이라 부르기도 한다. 이 색상들은 Xcode를 포함한 대부분 코드 편집기에서 해당 단계로 표시된다.
...
//    }
    ...
    override func setUp() {
        super.setUp() //슈퍼 클래스의 메서드를 먼저 호출하여 설정한다.
        availableFunds = 100
        itemCost = 42
    ...
    //이는 XCTest 프레임 워크 작동 방식 때문인데, 테스트 대상(target)내에서 XCTestCase의 subclass(여기서는 CashRegisterTests)를 인스턴스화하고
...
//여기서는 아래의 규칙에 따라 테스트 이름을 지정한다.
// • XCTest : 모든 테스트 메서드는 test로 시작한다.
// • test : 메서드의 메서드 이름을 이어서 지정해 준다. 여기서는 init 이다. 이후, 다음 부분과 구분하는 밑줄(underscore)을 추가해 준다.
// • 선택적으로 특별한 설정이 필요한 경우 이어서 설정한 후 다시 밑줄을 추가해 준다. 여기서는 사용하지 않는다.
// • 마지막으로 예상되는 결과를 추가한다. 여기서는 createsCashRegister이다.
...
//        transactionTotal = cost
        //매개변수로 전달된 cost를 transactionTotal로 설정한다. 이는 실제 앱에서 구현할 때는 틀린 로직이다.
...
// • Duplicate logic : 속성, 메서드, 클래스 등을 추축해, 중복을 제거한다.
// • Comments : 주석은 내용이 수행된 방식이 아니라, 수행된 이유를 설명해야 한다. 코드 작동 방식을 설명하는 주석은 제거한다.
//  코드 작동 방식 설명은 주석이 아닌 큰 메서드를 여러 메서드로 나누거나, 속성과 메서드의 이름을 보다 명확하게 구성해서 전달해야 한다.
...
//앱의 모든 기능을 완성하면, 제대로 테스트 된 시스템이 갖춰지게 된다. 현재 작은 단위의 TDD Cycle을 완료 했다.
//여기에 추가적인 기능을 구현하면서 cycle을 반복한다. 앞으로 추가해야 할 것의 목록은 다음과 같다.
// • availableFunds를 받는 initializer를 작성한다.
// • transaction에 추가되는 addItem 메서드를 작성한다.
...
//sut은 system under test의 약어이다. TDD에서 무엇을 테스트하는 지 나타내는 매우 일반적인 이름이다.
//아직 init(availableFunds:)를 작성하지 않았으므로 컴파일이 되지 않아(테스트 실패) Red 단계가 완료된다. 테스트 통과를 위해 해당 속성과, 생성자를 CashRegister에 추가해 준다.
//위의 코드를 추가해 주면, availableFunds 매개변수를 받아 초기화(initialize)할 수 있다. 컴파일하면, 두 테스트 모두 통과했으므로, Green 단계가 완료 된다.
...
//init(availableFunds:) 리팩토링 이후에도 testInitAvailableFunds가 테스트를 통과한다는 것은 변경 사항이 기존 기능에서 오류를 일으키지 않는다는 안정성을 보장해 준다. 이는 TDD의 주요 이점이다.
...
//TDDing addItem
//addItem으로, transaction에 비용을 추가한다. 항상 실패하는 테스트를 먼저 작성해야 한다.
extension CashRegisterTests {
...
//tearDown()은 각 테스트 메서드가 완료된 직후에 호출(분리) 된다. 이 메서드들은 코드 중복 해소를 위해, 해당 논리를 이동하기 적합한 장소이다.
//setUp() 내에서 설정한 속성들은 반드시 tearDown에서 nil이 되어야 한다.
//이는 XCTest 프레임 워크 작동 방식 때문인데, 테스트 대상(target)내에서 XCTestCase의 subclass(여기서는 CashRegisterTests)를 인스턴스화하고
//모든 테스트가 실행될 때 까지 해당 속성들을 해제하지 않는다. 따라서 tearDown에서 속성을 nil로 설정하지 않으면, 메모리 누수가 일어난다.
//테스트의 수가 많고, 제대로 속성을 해제 처리하지 않을 경우, 메모리 및 성능에 문제가 발생할 수 있다.
//이제 해당 속성을 사용해 중복된 로직을 제거한다.
//초기화 코드를 setup()으로 이동했기 때문에, 해당 메서드가 단순히 addItem(_:)를 구현하고 있다는 것을 분명하게 알 수 있다.
//리팩토링 이후에도 테스트가 통과하면 다음 TDD Cycle을 시작한다.
...
//이어서, 테스트가 통과하도록 코드를 수정한다. 콘솔의 에러 메시지를 확인해 보면, sut.transactionTotal와 expectedTotal이 일치하지 않기 때문에 테스트가 통과되지 않았음을 알 수 있다.
//이전에 테스트 통과를 위한 최소한의 코드만 작성하면서, 매개변수 cost를 transactionTotal로 그대로 할당해 일치하지 않는다. 이를 수정해 준다. 테스트가 통과하면, 리팩토링 단계로 넘어간다.
//itemCost 변수가 중복되므로 인스턴스 변수로 가져오고, setUp과 tearDown에서 추가적인 로직을 구현한다.
//itemCost는 CashRegisterTests의 인스턴스 속성만 있어야 한다. 나머지를 제거해 중복된 코드를 리팩토링한다.
//sut.addItem(itemCost)는 testAddItem_oneItem와 testAddItem_twoItems에서 반복된다. 하지만 이 코드는 제거해선 안 된다.
//setUp()은 모든 테스트 메서드가 실행되기 전에 실행된다. 하지만 testInitAvailableFunds는 addItem(_:)을 호출할 필요가 없으므로 해당 코드를 실행할 필요가 없다.
//TDD CashRegister를 계속 구현하다보면, addItem(_:)를 호출할 필요가 없는 다른 메서드가 추가 될 수도 있다.
//따라서 중복된다고 해서 모든 속성과 메서드를 setUp()로 이동시켜선 안 된다.
//TDD 리팩토링에서 필요한 경우, 이전 단계에서 최선이었던 코드를 변경하는걸 두려워할 필요 없다.




//Key points
//TDD Cycle은 4가지 단계가 있다.
// 1. Red : 실패하는 테스트를 작성한다.
// 2. Green : 테스트를 통과한다.
// 3. Refactor : 앱 코드와 테스트 코드를 정리한다.
// 4. Repeat : 모든 기능이 구현될 때 까지 반복한다.
